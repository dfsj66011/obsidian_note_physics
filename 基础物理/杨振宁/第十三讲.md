...
特殊情况一：当 $I_{1}=I_{2}=I_{3}$，这意味着 $\dot{\omega}_{1}=\dot{\omega}_{2}=\dot{\omega}_{3}=0$，即 $\omega_{1},\omega_{2},\omega_{3}$ 都是常数。
...
\end{align}$$
第一个式子说明 $\omega_{1}$ 是个常数，而第二、三个式子说明 $\dot{\omega}_{2}$ 等于某个常数乘以 $\omega_{3}$；而 $\dot{\omega}_{3}$ 恰好又是负的这个常数乘以 $\omega_{2}$，这意味它们是简谐的。
...
特殊情况三：$I_{1}, I_{2}, I_{3}$ 各不相等。

Special case three. I1 is not equal to I2.It's not equal to I3. It's not equal to I1. You can see that this is even more complicated than the second case.And that is the problem on which Madame Kovalevsky won that prize. （第三种特殊情况。I1不等于I2，不等于I3，也不等于I1。可以看到这比第二种情况更为复杂。而这就是科瓦列夫斯卡娅夫人获奖所解决的问题。）
...
定义$$\mathbf{J} = \sum_i m_i (r_i^2 \mathbf{I} - \vec{r}_i \vec{r}_i^T)$$为惯性矩矩阵或惯性张量（关于原点的，此处原点是质心）。
...
如果我们选取的坐标系 XYZ 是刚体关于质心的惯性主轴，那么 $\mathbf{J}$ 是对角矩阵：$$
...
斜坡倾斜角为 $\theta$，沿斜坡滚动距离为 $d$，则有 $h=d\sin \theta$，因此有：$$a=\frac{v^2}{2d}=\frac{g\sin \theta}{1+\frac{I}{mr^2}}$$在第 12 版本教材中，该公式标号为 (11.2.8)，课本第 315 页。
